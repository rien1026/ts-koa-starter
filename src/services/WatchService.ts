...
const createWatchJob = () => {
	...
	try {
		watchJob = watch(Constants.COMMON_INFO.IMAGE_PATH, { recursive: true, filter: /\.PNG$/i }, async (event, name) => {
			if (event === 'remove') {
				return;
			}
			...
			try {
				...
				logger.error('watchJobListenerErr : ' + err.message);
			}
		});
		return true;
	} catch (err) {
		logger.error('watchJobErr : ' + err.message);
		return false;
	}
};
